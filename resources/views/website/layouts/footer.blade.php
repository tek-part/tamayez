<!-- ===============>> footer  <<================= -->
<footer class="footer brand-1 mt-4">
    <div class="">
        <div class="footer__wrapper">
            <div class="footer__top mt-0 py-3 footer__top--style1">
                <div class="container">
                    <div class="row gx-4">
                        <div class="col-md-4">
                            <div class="footer__about">
                                <a style="width: 150px" href="#-2.html" class="footer__about-logo"><img
                                        src="public/logo.png" width="100" alt="Logo"
                                        style="max-width: 100%; width: 100px" /><img /></a>
                                <p class="footer__about-moto">
                                    حصل على نظامك الخاص في دقائق، واربط منتجاتك بمجموعة متكاملة من الحلول الرقميَّة الذكيَّة للمدفوعات، والشحن، وإدارة المخزون، والتسويق، بكل سهولة؛ لأن نجاحك لا يحتاج إلى تعقيد.
                                </p>
                                <ul class="d-flex align-items-center gap-2">
                                    <li>
                                        <a href="https://www.facebook.com/profile.php?id=61556684580004">
                                            <img src="./asset/facebook (4) 1.svg" alt="" />
                                        </a>
                                    </li>
                                    <li>
                                        <a
                                            href="https://www.linkedin.com/company/tek-part/mycompany/?viewAsMember=true">
                                            <img src="./asset/linkedin (1) 1.svg" alt="" />
                                        </a>
                                    </li>

                                </ul>
                             
                            </div>
                        </div>
                        <div class="col-md-2 col-sm-4 col-6 mt-4">
                            <div class="footer__links">
                                <div class="footer__links-tittle">
                                    <h6>الروابط</h6>
                                </div>
                                <div class="footer__links-content">
                                    <ul class="footer__linklist">
                                        <li>
                                            <a href="{{route('website.home')}}" class="active">الرئيسية </a>
                                        </li>
                                        <li>
                                            <a href="{{route('website.price')}}">الأسعار</a>
                                        </li>
                                        <li>
                                            <a href="{{route('website.contact')}}">تواصل معنا</a>
                                        </li>

                                    </ul>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-2 col-sm-4 col-6 mt-4">
                            <div class="footer__links">
                                <div class="footer__links-tittle">
                                    <h6>روابط اخري</h6>
                                </div>
                                <div class="footer__links-content">
                                    <ul class="footer__linklist">
                                        <li>
                                            <a href="{{ route('book') }}"> ابد مشروعك الان</a>
                                        </li>
                                        <li>
                                            <a href="{{ route('website.contact') }}">تواصل معنا  </a>
                                        </li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-3 col-sm-6 col-12 mt-4">
                            <div class="footer__links">
                                <div class="footer__links-tittle">
                                    <h6>معلومات التواصل
                                    </h6>
                                </div>
                                <div class="footer__links-content">
                                    <ul class="footer__linklist">
                                        <li class="footer__linklist-item d-flex gap-3 align-items-center">
                                            <a href="https://wa.me/966562005408"><img class="mx-2"
                                                    src="asset/phone.svg" alt="" />
                                                <span dir="ltr">+966 56 200 5408 </span>
                                            </a>
                                        </li>
                                        <li class="footer__linklist-item d-flex gap-3 align-items-center">
                                            <a href="https://a.me/201094260793 "><img class="mx-2"
                                                    src="asset/phone.svg" alt="" />
                                                <span dir="ltr">+201094260793 </span>
                                            </a>
                                        </li>
                                        <li class="footer__linklist-item d-flex gap-3 align-items-center">
                                            <a href="mailto:info@tek-part.com"><img class="mx-2" src="asset/sms.svg"
                                                    alt="" />
                                                <span dir="ltr">info@tek-part.com</span>
                                            </a>
                                        </li>
                                        <li class="footer__linklist-item d-flex gap-3 align-items-center">
                                            <a href="#"><img class="mx-2" src="asset/location.svg"
                                                    alt="" />
                                                المملكة العربية السعودية - الرياض </a>
                                        </li>

                                    </ul>
                                    <ul class="d-flex gap-3 my-4">
                                        <li>
                                            <a href="">
                                                <img src="asset/icon_behance.svg" alt="" width="35" />
                                            </a>
                                        </li>
                                        <li>
                                            <a href="">
                                                <img src="asset/icon_dribbble.svg" alt="" width="35" />
                                            </a>
                                        </li>
                                        <li>
                                            <a href="">
                                                <img src="asset/icon_facebook.svg" alt="" width="35" />
                                            </a>
                                        </li>
                                        <li>
                                            <a href="">
                                                <img src="asset/icon_linkedin.svg" alt="" width="35" />
                                            </a>
                                        </li>
                                        <li>
                                            <a href="">
                                                <img src="asset/icon_twitter.svg" alt="" width="35" />
                                            </a>
                                        </li>
                                        <li>
                                            <a href="">
                                                <img src="asset/icon_youtube.svg" alt="" width="35" />
                                            </a>
                                        </li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="footer__bottom">
                <div class="footer__end">
                    <div class="text-center w-100 m-auto">
                        <p class="mb-0">
                            <img src="asset/Star 18.svg" alt="" width="20" />
                            كل الحقوق محفوظه ل Tamayez 2024
                            <a href="https://tek-part.com/" target="_blank"> Tek Part </a>
                            2024

                        </p>
                        {{-- <p> @php
                            $visitors = App\Models\Visitor::count();
                        @endphp
                            {{ number_format($visitors, 0, '.', ',') }} زائر

                        </p> --}}
                    </div>
                </div>
            </div>
        </div>
    </div>
</footer>
<div class="social-icons"
    style="
        position: fixed;
        bottom: 20px;
        right: 20px;

        z-index: 999;
      ">
    <div class="sub-icons main-icon">
        <a href="https://wa.me/966562005408" target="_blank">
            <div class="sub-icon">
                <svg stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 448 512" height="2em"
                    width="2em" xmlns="http://www.w3.org/2000/svg">
                    <path
                        d="M380.9 97.1C339 55.1 283.2 32 223.9 32c-122.4 0-222 99.6-222 222 0 39.1 10.2 77.3 29.6 111L0 480l117.7-30.9c32.4 17.7 68.9 27 106.1 27h.1c122.3 0 224.1-99.6 224.1-222 0-59.3-25.2-115-67.1-157zm-157 341.6c-33.2 0-65.7-8.9-94-25.7l-6.7-4-69.8 18.3L72 359.2l-4.4-7c-18.5-29.4-28.2-63.3-28.2-98.2 0-101.7 82.8-184.5 184.6-184.5 49.3 0 95.6 19.2 130.4 54.1 34.8 34.9 56.2 81.2 56.1 130.5 0 101.8-84.9 184.6-186.6 184.6zm101.2-138.2c-5.5-2.8-32.8-16.2-37.9-18-5.1-1.9-8.8-2.8-12.5 2.8-3.7 5.6-14.3 18-17.6 21.8-3.2 3.7-6.5 4.2-12 1.4-32.6-16.3-54-29.1-75.5-66-5.7-9.8 5.7-9.1 16.3-30.3 1.8-3.7.9-6.9-.5-9.7-1.4-2.8-12.5-30.1-17.1-41.2-4.5-10.8-9.1-9.3-12.5-9.5-3.2-.2-6.9-.2-10.6-.2-3.7 0-9.7 1.4-14.8 6.9-5.1 5.6-19.4 19-19.4 46.3 0 27.3 19.9 53.7 22.6 57.4 2.8 3.7 39.1 59.7 94.8 83.8 35.2 15.2 49 16.5 66.6 13.9 10.7-1.6 32.8-13.4 37.4-26.4 4.6-13 4.6-24.1 3.2-26.4-1.3-2.5-5-3.9-10.5-6.6z">
                    </path>
                </svg>
            </div>
        </a>
    </div>
</div>
